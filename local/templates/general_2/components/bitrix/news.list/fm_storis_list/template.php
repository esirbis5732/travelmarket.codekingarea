		...
		<?foreach($arResult['ITEMS'] as $arItems):?>
			<div class="items row">
				<?foreach($arItems['ITEMS'] as $key => $arItem):?>
					...
					$defaultColor 		= ['r' => 255,'g' => 255,'g' => 255];
					$color 				= getAverageColor($_SERVER['DOCUMENT_ROOT'].$imageSrc, false) ?? $defaultColor;
					$colorText 			= (isDarken($color)) ? 'white' : 'black';
					$colorForCss 		= implode(',', $color);
					$gradientPrimary 	= "linear-gradient( to top, $colorForCss 0%, $colorForCss 50%,rgba($colorForCss,0.7) 70%,rgba($colorForCss,0.5) 100% );";
					$gradientElse 		= "linear-gradient( to right, $colorForCss 0%, $colorForCss 50%,rgba($colorForCss,0.7) 70%,rgba($colorForCss,0.5) 100% );";

					// if ($color)
						// $color = '#'.$color;

					// echo '<img src="'.$imageSrc.'"><br>';
					// echo '<a style="color: '.$color.'">'.$color.'</a>';
					...
					?>
							<a href="<?=$arItem['DETAIL_PAGE_URL']?>" class="item item_bottom shadow animation-boxs <?//=($isAjax ? 'preload-hidden' : '')?> <?=($isWideBlock ? 'wide-block' : '')?> <?=($hasWideBlock ? '' : 'normal-block')?>" style="color: <?=$colorText?>; background: rgb(<?=$colorForCss?>);"  id="<?=$this->GetEditAreaId($arItem['ID']);?>">
									...
									<?if($bImage):?>
										<div class="image shine">
											<?if($bDetailLink):?><div class="card"><?endif;?>
											    ...
											    <img rel="preload" as="image" class="stories_prev" src=<?=$imageSrc?> alt="<?=($bImage ? $arItem['DETAIL_PICTURE']['ALT'] : $arItem['NAME'])?>" title="<?=($bImage ? $arItem['DETAIL_PICTURE']['TITLE'] : $arItem['NAME'])?>"/>
											    <?if($bDetailLink):?>
                                                <canvas></canvas>
                                            </div><?endif;?>
										</div>
									<?endif;?>
                                            <div class="item__gradient" style="<?=$gradientPrimary?>">
                                                <div class="item__gradient_horizontal" style="<?=$gradientElse?>">
                                        <div class="title">
