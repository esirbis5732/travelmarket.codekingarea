...
<?
$arItemFilter = GetIBlockAllElementsFilter($arParams);
...
$itemsCnt = CIblockElement_GetList(array("CACHE" => array("TAG" => GetIBlockCacheTag($arParams["IBLOCK_ID"]))), $arItemFilter, array());?>
...
<?else:?>
	...
	if($arParams['USE_RSS'] !== 'N'){
		ShowRSSIcon($arResult['FOLDER'].$arResult['URL_TEMPLATES']['rss']);
	}
	?>
	<?$arSections = CIBLockSection_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => GetIBlockCacheTag($arParams['IBLOCK_ID']), 'GROUP' => array('ID'), 'MULTI' => 'N', 'URL_TEMPLATE' => $arResult['FOLDER'].$arResult['URL_TEMPLATES']['section'])), array_merge($arItemFilter, array('DEPTH_LEVEL' => 1, 'CNT_ACTIVE' => "Y")), false, array('ID', 'SECTION_PAGE_URL'));?>
	...
	if($arSections)
	{
		...
		{
			$arElements = CIblockElement_GetList(array('CACHE' => array('TAG' => GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'Y')), array_merge($arItemFilter, array("SECTION_ID" => $arSection["ID"])), false, false, array('ID', 'TAGS'));
			if(!$arElements)
	...
	{
		$arElements = CIblockElement_GetList(array('CACHE' => array('TAG' => GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'Y')), $arItemFilter, false, false, array('ID', 'TAGS'));
		foreach($arElements as $arElement)
		{
			if($arElement['TAGS'])
			{
				...
				<ul class="categorys">
					<?foreach($arSections as $arSection):
						if(isset($arSection['NAME']) && $arSection['NAME']):?>
							<li><a href="<?=$arSection['SECTION_PAGE_URL'];?>" class="dark-color"><span class="text"><?=$arSection['NAME'];?></span></a></li>
						<?endif;?>
					<?endforeach;?>
				</ul>
			</div>
