...
class BaseServiceRest extends BaseService
{
	 ...
	 * @return mixed
	 */
	protected function doRequest($url, array $params = array(), $type = 'get', $context = null, $options=null)
	{
		$result = null;
		try {
			switch ($type) {
				case 'post':
					...
					break;
				case 'get_s'://для тезтура, простой get запрос без заголовка
					$result = $this->doGetsRequest($url, $params);
					break;
				case 'get_header'://для тревелаты, запрос с указанием заголовка
					$result = $this->doGetHeaderRequest($url, $params, $options);
					break;
				case 'get'://для слетать ру
				default:
		...
		if ($type!=='get_s' && $type!=='get_header') return $this->parseResult($result);
		else return $result;
	}
	 ...
	 * @return array 
	 */
	protected function doGetsRequest($url, array $params = array())
	{
		$result = null;
		$requestContext = http_build_query($params);
		$result = file_get_contents($url.'?'.$requestContext, false);
		return $result;
	}
	protected function doGetHeaderRequest($url, array $params = array(), array $options = array())
	{
       $context = stream_context_create($options);
	   $requestContext = http_build_query($params);
       $result = file_get_contents($url.'?'.$requestContext, false, $context);
       return $result;
	}
	protected function doGetRequest($url, array $params = array(), $context = null)
	{
		...
		$requestUrl = $this->implodeUrl($arUrl);var_dump($requestUrl);
